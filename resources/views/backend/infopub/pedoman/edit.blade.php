...
<div class="card">
    ...
    <form class="form-validate-jquery" action="{{route('pedoman.update', encrypt($pedoman->id))}}" method="post" enctype="multipart/form-data" autocomplete="off">
        @csrf
        @method('PUT')
        <div class="card-body">
            <div class="mb-4">
                ...
                <div class="row mb-3">
                    ...
                    <div class="col-lg-10">
                        <input maxlength="255" value="{{ old('judul_pedoman') ?? $pedoman->judul_pedoman }}" type="text" name="judul_pedoman" class="form-control @error('judul_pedoman') is-invalid @enderror" required placeholder="Masukkan Judul Pedoman">
                        @error('judul_pedoman')
                ...
                <div class="row mb-3">
                    ...
                    <div class="col-lg-10">
                        <textarea maxlength="1000" name="deskripsi" class="form-control @error('deskripsi') is-invalid @enderror" placeholder="Deskripsi singkat pedoman (opsional)">{{ old('deskripsi') ?? $pedoman->deskripsi }}</textarea>
                        @error('deskripsi')
                        <div class="alert alert-danger mt-2">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="col-lg-10">
                        <select name="kategori" class="form-control form-control-select2 select @error('kategori') is-invalid @enderror">
                            ...
                            @foreach ($kategori as $item)
                                <option value="{{ $item->id_kategori }}" {{ (old('kategori') ?? $pedoman->kategori) == $item->id_kategori ? 'selected' : '' }}>{{ $item->nama_kategori }}</option>
                            @endforeach
                        </select>
                        @error('kategori')
                        <div class="alert alert-danger mt-2">{{ $message }}</div>
                ...
                <div class="row mb-3">
                    ...
                    <div class="col-lg-10">
                        <input value="{{ old('tanggal_terbit') ?? $pedoman->tanggal_terbit }}" type="date" name="tanggal_terbit" class="form-control @error('tanggal_terbit') is-invalid @enderror">
                ...
                <!-- File uploader -->
                <div class="row mb-3">
                    <label class="col-form-label col-lg-2">File Pedoman</label>
                    <div class="col-lg-10">
                        <input type="file" class="form-control @error('file') is-invalid @enderror" name="file">
                        <small class="text-muted">Kosongkan jika tidak ingin mengganti file. Format: doc, docx, ppt, pptx, csv, xls, xlsx, pdf, zip, rar. Maks 20MB.</small>
                        <div class="mt-2">
