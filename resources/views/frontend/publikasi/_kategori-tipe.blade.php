{{-- Partial bersama untuk halaman kategori publikasi per-tipe (berita/warta/artikel).
     Menerima: $items, $isSearch, $searchValue, $kategoriList, $tipeLabel, $tipeDesc,
               $indexRoute, $kategoriRoute, $searchParam, $listPartial --}}

@php $currentKategori = request()->route('kategori'); @endphp
...
<section class="bg-white py-16 sm:py-20 dark:bg-navy-950">
    <div class="fe-container">
        ...
        <div x-data="liveSearch({
                url: @js(route($kategoriRoute, $currentKategori)),
                param: @js($searchParam),
                target: '#{{ $searchParam }}-results',
                initialValue: @js($searchValue ?? ''),
                extra: { current_kategori: @js($currentKategori) },
            })" class="mt-10">
            ...
            <div class="mt-6 flex flex-wrap items-center justify-center gap-2">
                <a href="{{ route($indexRoute) }}" class="fe-pill">Semua</a>
                @foreach ($kategoriList as $item)
                    @php $slug = strtolower($item->nama_kategori); @endphp
                    <a href="{{ route($kategoriRoute, $slug) }}" class="fe-pill {{ $currentKategori === $slug ? 'is-active' : '' }}">
