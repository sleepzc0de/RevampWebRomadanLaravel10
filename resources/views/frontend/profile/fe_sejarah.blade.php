            ...
            <div class="row p-t-10 align-items-center">
                ...
                <div class="col-md-6 order-md-2">
                    @php
                        ...
                        // Check if we have media in the new JSON format
                        if(isset($item->media) && !empty($item->media)) {
                            $mediaItems = json_decode($item->media, true) ?? [];
                            $hasMedia = count($mediaItems) > 0;
                        }

                        // If we don't have media in the new format but have a legacy image
                        if(!$hasMedia && !empty($item->image)) {
                            $mediaItems = [
                                [
                                    'type' => 'image',
                                    'path' => $item->image,
                                    'original_name' => 'Gambar Sejarah'
                                ]
                            ];
                            $hasMedia = true;
                        }
                                ...
                                @if($media['type'] === 'image')
                                    <div class="media-slide">
                                        <a href="{{ asset('storage/romadan_gambar_web/' . $media['path']) }}"
                                           data-lightbox="sejarah-images"
                                           data-title="{{ $media['original_name'] ?? 'Gambar Sejarah' }}">
                                            <img src="{{ asset('storage/romadan_gambar_web/' . $media['path']) }}"
                                                 alt="{{ $media['original_name'] ?? 'Gambar Sejarah' }}">
                                        </a>
                                    </div>
