...
<style>
    ...
    .txt-visi-misi ol ol li {
        list-style-type: lower-alpha !important;
    }

    .txt-visi-misi ul ul li {
    ...
    .txt-visi-misi table td,
    .txt-visi-misi table th {
        padding: 0.5rem;
                    ...
                    <div class="media-slider">
                        ...
                        @if($item->images && $item->images->count() > 0)
                            @foreach($item->images->sortBy('sort_order') as $image)
                                <div class="media-slide">
                                    <a href="{{ asset('storage/romadan_gambar_web/' . $image->image) }}"
                                       data-lightbox="visimisi-images"
                                       data-title="{{ $item->judul }}">
...
<script>
    $(document).ready(function(){
        ...
        $('.media-slider').on('beforeChange', function(event, slick, currentSlide, nextSlide){
            // Check if current slide contains an iframe (video)
            const $currentSlide = $(slick.$slides[currentSlide]);
            if ($currentSlide.find('iframe').length > 0) {
                // Get the iframe's src
                const iframe = $currentSlide.find('iframe')[0];
                const src = iframe.src;
                // Reset the iframe to stop the video from playing
                iframe.src = src;
            }
        });
    });
